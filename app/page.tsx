...
const EBOOK_LINK = "https://pay.hotmart.com/tuebookaqui"; 

export default function TipherethV800() {
  ...
  const drawConnectors = (ctx: CanvasRenderingContext2D, landmarks: any[]) => {
      ...
      // Dibujamos solo contornos principales para no saturar
  ...
  const MagazineHeader = () => (
    <div className="flex justify-between items-end border-b border-black pb-4 mb-6 md:mb-10">
        <div>
            {/* Texto responsive: más pequeño en móvil, grande en PC */}
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white overflow-x-hidden">
      <style jsx global>{`
        ...
        /* En móvil se ve normal, al imprimir se fuerza el salto de página */
        ...
        @media print { 
            ...
            /* Al imprimir, forzamos la estructura de revista */
      ...
      {appMode === 'HOME' && (
        <div className="flex flex-col items-center justify-center h-screen bg-black text-white p-6">
             <div className="border border-white/20 p-8 md:p-12 max-w-lg w-full text-center backdrop-blur-sm">
                 {/* TÍTULO RESPONSIVE: text-5xl en movil, text-7xl en PC */}
                 <h1 className="text-5xl md:text-7xl font-serif italic mb-2 tracking-tighter">Tiphereth</h1>
                 <p className="text-[10px] text-gray-400 tracking-[0.4em] uppercase mb-12">The Harmony Station</p>
                 <div className="space-y-6">
                    <input onChange={e => setPatient({...patient, name: e.target.value})} className="w-full bg-transparent border-b border-white/40 p-3 text-center text-white outline-none placeholder:text-gray-600 font-serif text-xl uppercase" placeholder="SU NOMBRE" />
      ...
      {appMode === 'CAPTURE' && (
        <div className="relative w-full h-screen bg-black">
            ...
            <div className="absolute top-12 left-0 w-full text-center"><p className="text-white font-serif italic text-2xl md:text-3xl drop-shadow-lg">{captureStep === 0 ? "Retrato Frontal" : captureStep === 1 ? "Perfil Derecho" : "Perfil Izquierdo"}</p></div>
      ...
      {appMode === 'ANALYSIS' && (
        <div className="h-screen flex flex-col items-center justify-center bg-white text-black">
             <h2 className="font-serif text-3xl italic mb-6">Procesando...</h2>
      ...
      {appMode === 'RESULT' && (
        <div className="bg-[#f0f0f0] min-h-screen md:py-10">
            ...
            <div className="fixed bottom-6 right-6 z-50 no-print flex flex-col gap-3 items-end">
                 <button onClick={() => window.open(EBOOK_LINK)} className="bg-gray-800 text-white px-4 py-2 font-serif italic text-xs hover:scale-105 shadow-xl border border-gray-600">
                    📚 Ebook Casa
                 </button>
                 <div className="flex gap-2">
                    ...
                    <button onClick={() => window.open(`https://api.whatsapp.com/send?phone=${WS_NUMBER}&text=Hola Dr. Maya, vi mi revista digital y quiero agendar.`)} className="bg-white text-black border border-black px-6 py-3 font-serif italic text-sm hover:bg-gray-50 shadow-xl">
                        💬 Cita
                    </button>
                 </div>
            </div>
            ...
            <div className="w-full md:max-w-[210mm] mx-auto bg-[#fafafa] shadow-2xl page-break flex flex-col justify-between items-center text-center py-16 md:py-24 px-6 md:px-12 border-[10px] md:border-[20px] border-white outline outline-1 outline-gray-200">
                <div>
                    <p className="text-[10px] md:text-xs uppercase tracking-[0.4em] text-gray-400 mb-6">The Aesthetic Dossier</p>
                    {/* Título Responsive */}
                ...
                <div>
                    <p className="font-serif italic text-2xl md:text-3xl mb-3 text-gray-800 uppercase">The {patient.name || "Patient"} Edition</p>
            ...
            <div className="w-full md:max-w-[210mm] mx-auto bg-white shadow-2xl page-break">
                <MagazineHeader />
                <h2 className="text-3xl md:text-5xl font-serif italic text-center mb-4">I. El Lienzo</h2>
                <p className="text-center text-[10px] md:text-xs uppercase tracking-widest text-gray-500 mb-8">Análisis de Calidad Dérmica</p>
                
                {/* Layout: Columna en movil, Fila en PC/Print */}
                <div className="flex flex-col md:flex-row magazine-col gap-8 md:gap-12 items-start mb-12 px-4 md:pl-8">
                    <div className="w-full md:w-[45%] magazine-width shadow-xl md:rotate-[-2deg]">
                        <div className="aspect-[3/4] relative overflow-hidden bg-gray-100">
                             {/* OBJECT COVER SIEMPRE PARA EVITAR DISTORSIÓN */}
                    ...
                    <div className="w-full md:w-[55%] magazine-width flex flex-col justify-center pt-4 md:pt-8 md:pr-8">
                        <div className="mb-6 md:mb-8"><h3 className="font-bold uppercase text-xs md:text-sm mb-2 border-b-2 border-black inline-block pb-1">Análisis Espectral</h3><p className="text-sm md:text-base font-serif leading-relaxed text-gray-700">Utilizamos <strong>Espectrometría de Contraste Vascular</strong>. Revela la inflamación crónica y el daño solar.</p></div>
                        <div className="mb-6 md:mb-8"><h3 className="font-bold uppercase text-xs md:text-sm mb-2 border-b-2 border-black inline-block pb-1">Hallazgos</h3><p className="text-sm md:text-base font-serif leading-relaxed text-gray-700">Se detecta una barrera cutánea con signos de fatiga oxidativa y textura irregular en zona T.</p></div>
                    </div>
                </div>
                
                <div className="bg-[#fafafa] p-6 md:p-8 border-l-4 border-black mx-4 md:mx-8">
                    <h3 className="font-serif text-xl md:text-2xl italic mb-4">Diagnóstico & Tratamiento</h3>
                    <p className="text-sm md:text-base text-gray-700 mb-4 font-serif">Restauración profunda de la matriz extracelular.</p>
                    <div className="flex justify-between items-center border-t border-gray-300 pt-4"><span className="font-bold text-xs uppercase tracking-widest">Sugerencia:</span><span className="font-serif italic text-lg md:text-2xl">Láser + Bio-Revitalización</span></div>
                </div>
            </div>
            ...
            <div className="w-full md:max-w-[210mm] mx-auto bg-white shadow-2xl page-break">
                <MagazineHeader />
                <h2 className="text-3xl md:text-5xl font-serif italic text-center mb-4">II. La Escultura</h2>
                <p className="text-center text-[10px] md:text-xs uppercase tracking-widest text-gray-500 mb-8">Dinámica de Paquetes Grasos</p>
                <div className="relative mb-8 md:mb-12 mx-4 md:mx-8 shadow-2xl">
                     <div className="aspect-video w-full relative overflow-hidden bg-gray-100">
                         ...
                         {/* --- MANCHAS CORREGIDAS (ANATOMÍA) --- */}
                         {/* Pómulo: 48% altura */}
                         <div className="absolute top-[48%] left-[35%] w-16 h-12 bg-[#C4A484]/50 rounded-full blur-md border-2 border-[#C4A484]"></div>
                         {/* Jowl: 72% altura */}
                         <div className="absolute top-[72%] left-[40%] w-16 h-14 bg-black/40 rounded-full blur-xl border-2 border-black transform rotate-12"></div>
                         
                         <div className="absolute top-4 right-4 text-right bg-white/50 p-2 md:p-4 backdrop-blur-sm rounded">
                             <p className="text-[#C4A484] font-bold text-[10px] md:text-xs tracking-widest">▲ PÓMULO</p>
                             <p className="text-black font-bold text-[10px] md:text-xs mt-1 tracking-widest">▼ JOWL</p>
                         </div>
                     </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 magazine-col gap-6 md:gap-12 mb-8 mx-4 md:mx-8">
                    <div className="magazine-width"><h3 className="font-bold uppercase text-xs mb-2 border-b-2 border-black inline-block pb-1">Mapeo Volumétrico</h3><p className="text-sm font-serif leading-relaxed text-gray-700">Visualizamos cómo los compartimentos de soporte se atrofian y los superficiales caen.</p></div>
                    <div className="magazine-width"><h3 className="font-bold uppercase text-xs mb-2 border-b-2 border-black inline-block pb-1">Hallazgos</h3><p className="text-sm font-serif leading-relaxed text-gray-700">Deflación en tercio medio (pómulos), provocando descenso de tejidos.</p></div>
                </div>
                <div className="text-center border-y-2 border-black py-6 md:py-8 mx-4 md:mx-8">
                    <p className="font-serif italic text-xl md:text-3xl mb-2">"No es exceso de piel, es falta de soporte."</p>
                    <p className="text-[10px] md:text-sm uppercase tracking-[0.2em] font-bold">Tratamiento: Reposición Volumétrica</p>
                </div>
            </div>
            ...
            <div className="w-full md:max-w-[210mm] mx-auto bg-white shadow-2xl page-break flex flex-col">
                <MagazineHeader />
                <h2 className="text-3xl md:text-5xl font-serif italic text-center mb-4">III. Los Cimientos</h2>
                <p className="text-center text-[10px] md:text-xs uppercase tracking-widest text-gray-500 mb-8">Análisis del SMAS y Soporte Óseo</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 magazine-col gap-6 md:gap-8 mb-8 mx-4 md:mx-8 flex-grow">
                    <div className="bg-[#fafafa] p-6 flex flex-col magazine-width">
                        <h3 className="text-center font-bold text-xs uppercase tracking-widest mb-4">SMAS (Tensión)</h3>
                        ...
                        <p className="text-xs font-serif text-justify mt-auto"><strong>Dx:</strong> Laxitud ligamentaria y pérdida de tensión.</p>
                    </div>
                    <div className="bg-[#fafafa] p-6 flex flex-col magazine-width">
                        <h3 className="text-center font-bold text-xs uppercase tracking-widest mb-4">Soporte Óseo</h3>
                        <div className="aspect-square relative overflow-hidden mb-4 shadow-lg">
                        ...
                        <p className="text-xs font-serif text-justify mt-auto"><strong>Dx:</strong> Retracción en la apertura piriforme.</p>
                    </div>
                </div>

                <div className="bg-black text-white p-6 md:p-8 mx-4 md:mx-8 mb-8">
                    <p className="font-serif italic text-xl md:text-2xl mb-6 text-center">Su Plan de Transformación</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 magazine-col gap-6 border-t border-white/30 pt-6">
                        <div className="text-center md:text-right md:border-r md:border-white/30 md:pr-8 magazine-width">
                            <p className="text-xs font-bold uppercase text-[#C4A484] mb-2">Opción A: Médico</p>
                            <p className="text-xs font-light mb-4">Intervención experta.</p>
                            <button onClick={() => window.open(`https://api.whatsapp.com/send?phone=${WS_NUMBER}&text=Dr. Maya, quiero agendar mi cita médica.`)} className="bg-white text-black px-4 py-2 text-xs font-bold uppercase hover:bg-gray-200">Agendar Cita</button>
                        </div>
                        <div className="text-center md:text-left md:pl-4 magazine-width">
                            <p className="text-xs font-bold uppercase text-[#C4A484] mb-2">Opción B: Casa</p>
                            <p className="text-xs font-light mb-4">Guía digital de cuidados.</p>
                            <button onClick={() => window.open(EBOOK_LINK)} className="border border-white text-white px-4 py-2 text-xs font-bold uppercase hover:bg-white hover:text-black transition-colors">Comprar Ebook</button>
                        </div>
                    </div>
                    <div className="mt-8 text-center"><p className="text-[10px] md:text-sm font-bold uppercase tracking-[0.3em] inline-block">Aprobado por: {DR_NAME}</p></div>
                </div>
            </div>
